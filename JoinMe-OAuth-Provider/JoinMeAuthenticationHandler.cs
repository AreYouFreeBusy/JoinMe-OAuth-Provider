...
{
    public class JoinMeAuthenticationHandler : AuthenticationHandler<JoinMeAuthenticationOptions>
    {
        // see https://developer.join.me/docs/read/authorization/authorization_code_grant for docs 
        private const string AuthorizeEndpoint = "https://secure.join.me/api/public/v1/auth/oauth2";
        private const string TokenEndpoint = "https://secure.join.me/api/public/v1/auth/token";
        private const string UserInfoEndpoint = "https://api.join.me/v1/user";
        private const string XmlSchemaString = "http://www.w3.org/2001/XMLSchema#string";

        private readonly HttpClient _httpClient;
        private readonly ILogger _logger;

        public JoinMeAuthenticationHandler(HttpClient httpClient, ILogger logger)
        {
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                var context = new JoinMeAuthenticatedContext(Context, accessToken, expires, refreshToken, userJson);
                ...
                {
                    context.Identity.AddClaim(
                        new Claim(ClaimsIdentity.DefaultNameClaimType, context.FullName, XmlSchemaString, Options.AuthenticationType));
                }
                ...
                if (!string.IsNullOrEmpty(context.AccountType)) 
                {
                    context.Identity.AddClaim(
                        new Claim(ClaimTypes.Role, context.AccountType, XmlSchemaString, Options.AuthenticationType));
                }
        ...
        private async Task<bool> InvokeReplyPathAsync()
        {
            if (Options.CallbackPath.HasValue && Options.CallbackPath == Request.Path)
            {
                ...
                var context = new JoinMeReturnEndpointContext(Context, ticket);
